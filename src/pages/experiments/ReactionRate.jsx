...
const ReactionRate = () => {
  ...
  const experiment = experiments.find(e => e.id === 'reaction-rate')
  ...
  useEffect(() => {
    if (isSimulationMode) {
      logSimulationStart('reaction-rate')
      return () => {
        const timeSpent = Math.floor((Date.now() - simulationStartTime) / 1000)
        logSimulationEnd('reaction-rate', timeSpent)
      }
    }
