...
const BufferCapacity = () => {
  ...
  const experiment = experiments.find(e => e.id === 'buffer-capacity')
  ...
  useEffect(() => {
    if (isSimulationMode) {
      logSimulationStart('buffer-capacity')
      return () => {
        const timeSpent = Math.floor((Date.now() - simulationStartTime) / 1000)
        logSimulationEnd('buffer-capacity', timeSpent)
      }
    }
